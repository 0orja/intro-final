...
"""
Oorja and Yao
This final project is done by pygame. A gun is used to "kill" flies in marketplace on campus. Flies get hit by the gun and a dead fly image is displayed. The flies bounce around on the screen. Game ends when the gun accidentally hits a cat which runs horizontally on the screen. A mouse is used to control the gun and shoot the gun. Background music and the different music when the shooting occurs are implemented. There is also a counter to count the number of flies that are killed. In the end, the number is displayed, and a return button is created to go back into the game. The program can be called in terminal. """
...
class Button: #a class Button is created so that users can choose to see the introduction or go back to the game
    ...
    def clicked(self):
        ...
        if click[0] and onButton:
            ...
            return True
class Pair: #Pair is created for the random bouncing of the flies
...
class Gun: 
    ...
    def display(self, x, y, status):
        ...
        if self.status == "idle":
            ...
            screen.blit(self.image, (x-(gun_dm["width"]/2), y-(gun_dm["height"]/2)))
class Fly: #there are 4 images of the fly where they are displayed one by one to created the motion of a flying fly
    def __init__(self):
        ...
        if self.alive == True:
            ...
            if self.ani_speed == 0:
                if self.ani_pos < 4:
                    ...
                    self.ani_pos = 0
                self.ani_speed = 5
    def scare(self): #when they are scared, or shot by a gun, they stop moving a sound of a cat is played
        self.alive = False 
        self.velocity = Pair(0,0)
...
while True: #the main program
    ...
    if not finished and not home and not info:
        ...
        gun.update()
    if finished and not home and not info: #if the game ends(when cat is hot), the images display and the messages display and also a button will be there to go back to the game
